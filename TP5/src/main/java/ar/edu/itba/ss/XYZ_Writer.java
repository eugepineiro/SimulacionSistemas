...
public class XYZ_Writer {
    private static class PCoords {
        ...
        public PCoords(long id, double x, double y, double vx, double vy, double radius) {
            this.id = id;
            this.x = x;
            this.y = y;
            this.vx = vx;
        ...
        public String toString() {
            return String.format("%s\t%g\t%g\t%g\t%g\t%g\n", id, x, y, vx, vy, radius);
        }
    }
    ...
    public XYZ_Writer addFrame(Frame frame) {
        List<PCoords> f = frame.getParticles().stream().map(p -> new PCoords(p.getId(), p.getX(), p.getY(), p.getVx(), p.getVy(), p.getRadius())).collect(Collectors.toList());
