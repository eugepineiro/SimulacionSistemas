...
public class Runner {
    ...
    private static final String         JUPITER_POSTPROCESSING_FILENAME_MULTIPLE_DT        = "SdS_TP4_2021Q2G01_jupiter_results_with_multiple_dt";
        ...
        try {
            ...
            ///////////////////////////    JUPITER     ///////////////////////////
                ...
                if (config.getMultiple_dt().isActivated()) {
                    multipleRuns = true;
                    final MultipleDt mul = config.getMultiple_dt();
                    runJupiterSimulationWithMultipleDt(mul.getMin_exp(), mul.getMax_exp(), mul.getIncrement(), config);
                }
                if (config.getMultiple_velocities().isActivated()) {
                    multipleRuns = true;
                    final MultipleVelocities mul = config.getMultiple_velocities();
...
//            .writeAndClose();
    ...
    private static void runJupiterSimulationWithMultipleDt(double minExp, double maxExp, double increment, Config config) throws IOException {

        LocalDateTime launchDate = config.getLaunch_date();

        System.out.printf("Running jupiter simulation with multiple dt between exponent %.2g and %.2g with an increment of %.2g\n", minExp, maxExp, increment);
        ...
        new JsonWriter(JUPITER_POSTPROCESSING_FILENAME_MULTIPLE_DT)
                .withObj(results)
                .write();

        System.out.println("Finished saving " + JUPITER_POSTPROCESSING_FILENAME_MULTIPLE_DT + ".json");
    }
