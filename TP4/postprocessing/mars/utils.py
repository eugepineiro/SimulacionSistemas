...
def get_min_distances(planet_positions_by_date, spaceship_positions_by_date): # positions = [[x,y] , [x,y]]

    min_distances_by_date = {}

    for date in planet_positions_by_date.keys():

        min_distance = math.inf
        
        planet_positions = planet_positions_by_date[date]
        spaceship_positions = spaceship_positions_by_date[date]

        for i in range(len(planet_positions)):
            
            planet_position = planet_positions[i]
            spaceship_position = spaceship_positions[i]

            distance = calculate_distance(planet_position, spaceship_position)
